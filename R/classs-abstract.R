#' @noRd
#' @title Internal Class definition for `AbstractFArray`
...
AbstractFArray <- R6::R6Class(
  ...
  public = list(
    ...
    `@chunk_map` = function(
      ...
      nchunks <- chunkf$nchunks

      # try to get which index to iterate
      ...
      if(idx == 1){
        ...
        csize <- ceiling(dim[idx] / nchunks)
        nchunks <- ceiling(dim[idx] / csize)
        pdim <- dim[seq_len(idx-1)]
        psize <- prod(pdim)
        dim(self_m) <- c(pdim, self$partition_length / psize, self$npart)
        dim <- dim(self_m)
        ndims <- length(pdim) + 2
        sub_idx <- replicate(ndims, {
          get_missing_value()
        })
        if(!isTRUE(partitions == 'all') && length(partitions)){
          partitions <- as.integer(partitions)
          sub_idx[[ndims]] <- partitions
          nparts <- length(partitions)
        } else {
          nparts <- self$npart
        }

        lapply2(seq_len(nchunks), function(ii){
          mar1 <- (ii - 1) * csize + 1
          mar2 <- ii * csize
          if( mar2 > dim[ndims - 1] ){ mar2 <- dim[ndims - 1] }
          id1 <- (ii-1) * psize * csize + 1
          id2 <- id1 - 1 + psize * (mar2 - mar1 + 1)

          sub_idx[[idx]] <- quote(seq.int(mar1, mar2))
          sub_idx[['reshape']] <- c(id2 - id1 + 1, nparts)
          as.call(c(list(quote(`[`), quote(self_m)), sub_idx))
          expr <- as.call(c(list(quote(`[`), quote(self_m)), sub_idx))
          # assign('ee', environment(), envir = globalenv())
          map_f(eval(expr),ii,c(id1, id2))
        })
